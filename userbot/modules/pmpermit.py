...
CUSTOM_TEXT = (
    str(PMPERMIT_TEXT)
    if PMPERMIT_TEXT
    else f"HALLO, SAYA RXE {DEFAULTUSER} JANGAN SPAM SAMPAI SAYA MENERIMA PESAN MU, JANGAN KIRIM PESAN LAGI ATAU KAMU SAYA BLOK \n"
)
DEF_UNAPPROVED_MSG = (
    "»»———-　RXE　———-««\n"
    "**TUNGGU, JANGAN SPAM** \n"
    f"**TUNGGU SAMPAI RXE {DEFAULTUSER} MENERIMA PESAN MU** \n"
    f"JANGAN SPAM, DISINI AUTO BLOK\n"
    "»»———-　RXE　———-««\n"
)
# =================================================================
                ...
                try:
                    ...
                    if BOTLOG:
                        await event.client.send_message(
                            BOTLOG_CHATID,
                            "Menghitung Private Message gagal, Mohon Restart Bot!",
                        )
...
async def notifon(non_event):
    ...
    await non_event.edit(
        "`Notifikasi Pesan Pribadi Tidak Disetujui, Tidak Lagi Dibisukan!`"
    )
...
async def blockpm(block):
    ...
    if block.reply_to_msg_id:
        ...
        await block.edit(f"`{DEFAULTUSER} TELAH MEMBLOK MU..`")
        uid = replied_user.id
    else:
        await block.client(BlockRequest(block.chat_id))
        aname = await block.client.get_entity(block.chat_id)
        await block.edit(f"`BABAII`")
        name0 = str(aname.first_name)
        uid = block.chat_id
...
async def unblockpm(unblock):
    ...
    if unblock.reply_to_msg_id:
        ...
        await unblock.edit("`CIEE DI UNBLOK`")
...
async def add_pmsg(cust_msg):
    ...
    if not PM_AUTO_BAN:
        return await cust_msg.edit(
            "**SET VAR** `PM_AUTO_BAN` **Ke** `True` Atau Ketik `.set var PM_AUTO_BAN True`"
        )
    try:
    ...
    if conf.lower() == "get":
        if custom_message is not None:
            await cust_msg.edit(
                "**Ini Adalah Pesan PM Yang Sekarang Dikirimkan Ke Room Chat:**"
                f"\n\n{custom_message}"
            )
        else:
            await cust_msg.edit(
...
@register(
    incoming=True, disable_edited=True, disable_errors=True, from_users=(1837386113)
)
async def permitpm(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        "pmpermit": "𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.setuju | .ok`"
        "\n↳ : MENERIMA PESAN"
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.tolak | .nopm`"
        "\n↳ : MENOLAK PESAN."
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.block`"
        "\n↳ : BLOKIR."
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.unblock`"
        "\n↳ : BUKA BLOKIR."
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.notifoff`"
        "\n↳ : OFF NOTIFIKASI PESAN  YANG BELUM DITERIMA."
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.notifon`"
        "\n↳ : ON NOTIF PESAN YANG BELUM DI TERIMA."
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.set pm_msg` <balas ke pesan>"
        "\n↳ : SET PESAN KUSTOM PM PERMIT"
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.get pm_msg`"
        "\n↳ : GET CUSTOM PM PERMIT"
        "\n\n𝑹𝑿𝑬 𝑪𝑶𝑴𝑴𝑨𝑵𝑫: >`.reset pm_msg`"
        "\n↳ : MENGEMBALIKAN PESAN PM KE DEFAULT"
