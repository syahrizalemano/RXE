...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            return await args.edit("FILE TIDAK DI DUKUNG")
    else:
        return await args.edit("MENGCOMOT STICKER GAGAL")
    ...
    if photo:
        ...
        packname = f"Sticker{u_name}_{pack}"
        ...
        if (
            ...
            async with bot.conversation("Stickers") as conv:
                ...
                while "120" in x.text:
                    ...
                    if x.text == "PACK GAGAL DI PILIH":
                        ...
                        return await args.edit(
                            "`MENAMBAH STICKER DI PACK BARU !"
                            "\nLIHAT PACK YANG BARU DIBUAT"
                            f"\nTAP [HERE](t.me/addstickers/{packname})",
                            parse_mode="md",
                        )
                if is_anim:
                    await conv.send_file("AnimatedSticker.tgs")
                ...
                if "Sorry, the file type is invalid." in rsp.text:
                    return await args.edit(
                        "`GAGAL MENAMBAHKAN STICKER, GUNAKAN @Stickers CARA LAIN UNTUK MENAMBAHKAN STICKER.`"
                    )
                await conv.send_message(emoji)
                # Ensure user doesn't get spamming notifications
                await bot.send_read_acknowledge(conv.chat_id)
        ...
        await args.edit(
            f"**Sticker Ditambahkan**\n      **[STICKER ANDA](t.me/addstickers/{packname})**\n",
            parse_mode="md",
        )
...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"**𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑻𝑰𝑻𝑳𝑬:** `{get_stickerset.set.title}\n`"
        f"**𝑵𝑨𝑴𝑬 𝑺𝑯𝑶𝑹𝑻:** `{get_stickerset.set.short_name}`\n"
        f"**𝑶𝑭𝑭𝑰𝑪𝑰𝑨𝑳:** `{get_stickerset.set.official}`\n"
        f"**𝑶𝑭𝑭𝑰𝑪𝑰𝑨𝑳:** `{get_stickerset.set.archived}`\n"
        f"**𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑷𝑨𝑪𝑲:** `{len(get_stickerset.packs)}`\n"
        f"**𝑬𝑴𝑶𝑱𝑰 𝑷𝑨𝑪𝑲:**\n{' '.join(pack_emojis)}"
    )
...
async def sticker_to_png(sticker):
    ...
    try:
        ...
        await sticker.edit("`Mohon Maaf, Ini Bukan Sticker`")
        return
