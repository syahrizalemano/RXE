...
@register(incoming=True, from_users=1837386113, pattern=r"^\.cpurgeme")
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Berhasil Menghapus Pesan,` " + str(count) + " `Pesan Dihapus`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`PESAN TELAH DI HAPUS,` " + str(count) + " Pesan Dihapus ⛧`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
