...
async def locks(event):
    ...
    if input_str == "msg":
        ...
        if not input_str:
            await event.edit("`LOCK APAAN?`")
            return
        else:
            await event.edit(
                f"`OTAKLU NGEBUG, MAU SET LOCK YANG MANA TOLOL!` `{input_str}`"
            )
            return
    ...
    try:
        ...
        await event.edit(f"`HALO {what} GUA LOCK DULU YA!!`")
    except BaseException as e:
        await event.edit(f"`ETSS ADMIN YA? ?`\n**Kesalahan:** {str(e)}")
        return
