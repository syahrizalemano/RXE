...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`MENGAKTIFKAN PERINTAN GLOBAL BANNED`")
    else:
        dark = await dc.edit("`GLOBAL BANNED DI PROSES!!`")
        dark = await dc.edit("`GLOBAL BANNED AKTIF!!`")
    me = await userbot.client.get_me()
    await dark.edit(f"`GLOBAL BANNED SUDAH BERJALAN!!!`")
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit(
                f"`BENTAR, DIA ADALAH PEMBUATKU. GLOBAL BANNED DIBATALKAN`"
            )
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"`GLOBALL BANNED ON`")
            except BaseException:
                b += 1
    else:
...
async def gunben(userbot):
    ...
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`GLOBAL BAN DI BATALKAN`")
    else:
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit(
                "TIDAK ADA PERINTAH YANG DISETUJUI, DIA ADALAH PEMBUATKU"
            )
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"`GLOBAL BANNED DI BATALKAN. `")
            except BaseException:
                b += 1
    else:
    ...
    try:
        if ungmute(user.id) is False:
            return await dark.edit(
                "**Kesalahan! PePENGGUNA TIDAK ADA PADA DAFTAR GLOBAL BAN.**"
            )
    except BaseException:
        pass
    return await dark.edit(
        f"**Perintah :** `{ALIVE_NAME}`\n**USER:** [{user.first_name}](tg://user?id={user.id})\n**Aksi:** `GLOBAL BAN DI BATALKAN`"
    )
